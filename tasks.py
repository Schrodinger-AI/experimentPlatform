...
def generate_images(prompts, experiment_id):
    ...
    for i, prompt in enumerate(prompts):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            base64image, revised_prompt = future.result(timeout=timeout)
        ...
        if data_entry:
            ...
            data_entry['generate_date'] = int(time.time())
...
if __name__ == '__main__':
    ...
    sampleNum = 1
    generate_images(["I NEED to test how the tool works with extremely simple prompts. DO NOT add any detail, just use it AS-IS: A simple pixel art image of a The breed for this cat character is Oriental Shorthair cat in a natural sitting pose with 4 legs visible, facing directly at the viewer, with  Velvet Camisole, Galaxy Swirl Pad, Cap Forward, Hockey Stick, Obelisk Belt, Peppermint Twist Boots,and Baby Cactus. The image has a have an Ethereal Fog background background."],201)
